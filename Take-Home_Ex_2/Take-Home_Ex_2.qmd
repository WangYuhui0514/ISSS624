...
hexagon_busstop_tidy <- hexagon_busstop %>%
  st_drop_geometry() %>%
  select(FID, SCHOOL_COUNT, HDB_COUNT, BUSINESS_COUNT, ENTERTN_COUNT,  FB_COUNT,  FINANCE_COUNT, LR_COUNT, RETAIL_COUNT)
```
...
```{r}
flow_data <- flow_data %>%
  mutate(across(c(SCHOOL_COUNT, HDB_COUNT, BUSINESS_COUNT, ENTERTN_COUNT,  FB_COUNT,  FINANCE_COUNT, LR_COUNT, RETAIL_COUNT), ~ ifelse(. == 0, 0.99, .)))
